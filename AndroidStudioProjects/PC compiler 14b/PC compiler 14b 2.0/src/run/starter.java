...
//				"}";
		...
		for (Object sfunction : splittedfunction) {
			...
			for (Object body : structuredFunctionBody) {
				String[] structures = (String[])body;
				if (structures.length == 1) {
					if (structures[0].length() == 0 
							|| structures[0].startsWith("//")
							    || structures[0].startsWith("/*")) {
						continue;
					}
				}
				asmcode += SC.cStructuretoAsm(structures);
			}
		}
