...
public class BanDoActivity extends AppCompatActivity implements OnMapReadyCallback, LocationListener {
                ...
                txtTemp.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        ((ClipboardManager) getSystemService(CLIPBOARD_SERVICE))
                                .setPrimaryClip(ClipData.newPlainText("simple text", info));
                    }
                }, 500);
            }
        });
    ...
    private void khoiTaoBanDo_TheoDoiDuongDi(GoogleMap googleMap) {
        ...
        mMap.setOnMyLocationButtonClickListener(new GoogleMap.OnMyLocationButtonClickListener() {
            @Override
            public boolean onMyLocationButtonClick() {
                Toast.makeText(BanDoActivity.this, R.string.btnMyLocationGM_UpdatingPosition,
    ...
    public void onLocationChanged(Location location) {
        if (currentPosition != null)
            MyGoogleMap.showDirectionArrow(mMap, currentPosition, location);
        currentPosition = new LatLng(location.getLatitude(), location.getLongitude());
        if (currentTabId.equals((getResources().getString(R.string.TabHost_tabtitle_timdiadiem_bando)))) {
            MyGoogleMap.zoomCameraTo(mMap, currentPosition, sbrZoom_TabTimDiaDiem.getProgress() + 2);
            String info = MyGoogleMap.getAddressFromLocation(currentPosition.latitude,
                    currentPosition.longitude, BanDoActivity.this);
    ...
    public void onStatusChanged(String provider, int status, Bundle extras) {
        Log.i("MyTag", "onStatusChanged");
    }

    @Override
    public void onProviderEnabled(String provider) {
        Log.i("MyTag", "onProviderEnabled");
    }

    @Override
    public void onProviderDisabled(String provider) {
        Log.i("MyTag", "onProviderDisabled");
    }
                    ...
                    .setPositiveButton(R.string.AlertDialog_buttonSave_SaveMapInstruction_TimDuongDi,
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    if (MyFileIO.makeFolder(saveTask.getFolderName()))
                                        saveTask.execute();
