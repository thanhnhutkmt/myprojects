...
public class BallGameActivity extends AppCompatActivity {
     ...
     */
    private Thread playSoundEffect_ThreadPool[] = new Thread[3];
    private Integer thread_id = 0;
    private void runGame() {
        final Thread playSoundEffect = new Thread(new Runnable() {
            @Override public void run() {
//                MyMedia.playSoundEff(BallGameActivity.this, 5);
                MyMedia.playSound(BallGameActivity.this, 5);
                synchronized (thread_id) {
                    if (thread_id > 0) thread_id--;
                }
            }
        });
        for (int i = 0; i < 3; i++) playSoundEffect_ThreadPool[i] = playSoundEffect;
        ...
        img.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (thread_id < 3) {
                    playSoundEffect_ThreadPool[thread_id] = playSoundEffect;
                    playSoundEffect_ThreadPool[thread_id].start();
                }
                synchronized (thread_id) {
                    if (thread_id < 3) thread_id++;
                }
                score++;
                oa.cancel();
            }
